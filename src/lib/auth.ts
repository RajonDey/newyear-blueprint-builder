...
function tokenUserId(token: { id?: unknown; sub?: unknown }): string | undefined {
  const id = typeof token.id === "string" && token.id.length > 0 ? token.id : undefined
  if (id) return id
  const sub = typeof token.sub === "string" && token.sub.length > 0 ? token.sub : undefined
  return sub
}
      ...
      if (uid && stale) {
        ...
        try {
          ...
          if (dbUser) {
            ...
            console.warn("[auth] jwt refresh: no user row for token id; keeping session")
            token.roleSyncedAt = Date.now()
            return token
          }
        } catch (err) {
