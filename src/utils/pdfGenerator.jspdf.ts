...
export const generateSuccessPDF_jsPDF = (data: PDFData) => {
  try {
    ...
    centerText(`${APP_CONFIG.year}`, y, 85, 'helvetica', 'bold', [99, 102, 241]);
    ...
    centerText('PERSONAL STRATEGY REPORT', y, 13, 'helvetica', 'normal', [100, 116, 139]);
    ...
    centerText(new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }), pageHeight - 35, 10, 'helvetica', 'normal', [148, 163, 184]);
    ...
    data.goals.forEach((goal, index) => {
      ...
      doc.setFont('helvetica', 'bold');
      doc.text(goal.category.toUpperCase(), margin + 25.5, y + 15, { align: 'center' });
      ...
      const checkInLines = doc.splitTextToSize(goal.monthlyCheckIn, pageWidth - (margin * 2) - 20);
