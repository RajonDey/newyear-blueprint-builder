...
export const generateSuccessPDF = (data: PDFData) => {
  ...
  doc.text('Your 2025 Success Blueprint', pageWidth / 2, 20, { align: 'center' });
  ...
  doc.text(
    'Generated by Your 2025 Success Blueprint',
    pageWidth / 2,
    pageHeight - 10,
    { align: 'center' }
  );
  ...
  // Save the PDF
  try {
    const fileName = `${data.userName.replace(/\s+/g, '_')}_2025_Success_Blueprint.pdf`;
    doc.save(fileName);
