  ...
  if (checkIns.length === 0) {
    const empty = (
      <p className="text-sm text-muted-foreground">
        ...
        this project&apos;s momentum.
      </p>
    )

    if (embedded) {
      return (
        <div className="space-y-3 border-t border-border pt-5 mt-5">
        ...
        {checkIns.slice(0, 5).map((ci) => (
          ...
          >
            ...
            <div className="pb-4 min-w-0 flex-1">
              ...
              {ci.notes && <p className="text-sm mt-0.5">{ci.notes}</p>}
              {ci.blockers && (
    ...
    return (
      <div className="border-t border-border pt-5 mt-5 space-y-4">
        <p className="text-sm font-medium">
          Weekly progress · {checkIns.length} week{checkIns.length !== 1 ? "s" : ""}
        </p>
        {chart}
      </div>
    )
    ...
    <Card>
      <CardHeader className="pb-3">
        ...
        <p className="text-sm text-muted-foreground font-normal">
          From your weekly reviews — {checkIns.length} week
          {checkIns.length !== 1 ? "s" : ""} tracked
        </p>
      </CardHeader>
      <CardContent className="space-y-1">{chart}</CardContent>
    </Card>
  )
}
